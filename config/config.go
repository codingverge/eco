...
func New(ctx context.Context, schema []byte, modifiers ...Option) (*Config, error) {
...
func (p *Config) SkipValidation() bool {
...
func (p *Config) DirtyPatch(key string, value any) error {
	p.l.Lock()
	defer p.l.Unlock()
...
func (p *Config) BoolF(key string, fallback bool) bool {
	p.l.RLock()
	defer p.l.RUnlock()
...
func (p *Config) StringF(key string, fallback string) string {
	p.l.RLock()
	defer p.l.RUnlock()
...
func (p *Config) StringsF(key string, fallback []string) (val []string) {
	p.l.RLock()
	defer p.l.RUnlock()
...
func (p *Config) IntF(key string, fallback int) (val int) {
	p.l.RLock()
	defer p.l.RUnlock()
...
func (p *Config) Float64F(key string, fallback float64) (val float64) {
	p.l.RLock()
	defer p.l.RUnlock()
...
func (p *Config) DurationF(key string, fallback time.Duration) (val time.Duration) {
	p.l.RLock()
	defer p.l.RUnlock()
...
func (p *Config) ByteSizeF(key string, fallback bytesize.ByteSize) bytesize.ByteSize {
	p.l.RLock()
	defer p.l.RUnlock()
...
func (p *Config) GetF(key string, fallback interface{}) (val interface{}) {
	p.l.RLock()
	defer p.l.RUnlock()
...
func (p *Config) PrintHumanReadableValidationErrors(w io.Writer, err error) {
	p.printHumanReadableValidationErrors(p.Koanf, w, err)
}

func (p *Config) printHumanReadableValidationErrors(k *koanf.Koanf, w io.Writer, err error) {
	if err == nil {
		return
	}
...
func (p *Config) newKoanf() (_ *koanf.Koanf, err error) {
...
func (p *Config) createProviders(ctx context.Context) (providers []koanf.Provider, err error) {
...
func (p *Config) validate(k *koanf.Koanf) error {
	if p.skipValidation {
		return nil
	}
...
func (p *Config) replaceKoanf(k *koanf.Koanf) {
	p.Koanf = k
}

func (p *Config) watchForFileChanges(ctx context.Context, c watcherx.EventChannel) {
	for {
		select {
		case <-ctx.Done():
			return
...
func (p *Config) runOnChanges(e watcherx.Event, err error) {
...
func (p *Config) reload(e watcherx.Event) {
	p.l.Lock()
