...
func TestJSONFormatter(t *testing.T) {
	t.Run("pretty=true", func(t *testing.T) {
		l := New(ForceFormat("json_pretty"), ForceLevel(logrus.DebugLevel))
		var b bytes.Buffer
		l.Logrus().Out = &b
	...
	t.Run("pretty=false", func(t *testing.T) {
		l := New(ForceFormat("json"), ForceLevel(logrus.DebugLevel))
		var b bytes.Buffer
		l.Logrus().Out = &b
...
func TestGelfFormatter(t *testing.T) {
	t.Run("gelf formatter", func(t *testing.T) {
		l := New(ForceFormat("gelf"), ForceLevel(logrus.DebugLevel))
		var b bytes.Buffer
		l.Logrus().Out = &b
...
func TestTextLogger(t *testing.T) {
	...
	for k, tc := range []struct {
		...
		{
			l: New(ForceFormat("text"), LeakSensitive(), ForceLevel(logrus.DebugLevel)),
			expect: []string{
				"audience=application",
				"An error occurred.",
		...
		{
			l: New(ForceFormat("text"), ForceLevel(logrus.DebugLevel)),
			expect: []string{
